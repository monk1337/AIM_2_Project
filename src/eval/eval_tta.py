"""TTA on best ensemble: original + horizontal flip → average pred_3d."""
import os
import sys
import json
import numpy as np
...
def crop_for_honet(img, bbox, image_size=256, hand_side="right"):
...
def predict_honet(rows, model, batch_size=64, tta=False):
    from torch.cuda.amp import autocast
    out = {}
    for tta_flip in ([False, True] if tta else [False]):
        patches, meta = [], []
        for r in rows:
        ...
        bs = batch_size
        for i in tqdm(range(0, len(patches), bs), desc=f"HONet{'-TTA' if tta_flip else ''}"):
            ...
            for j, m in enumerate(meta[i:i+bs]):
                ...
                if key in out:
                    out[key] = (out[key] + p3d) / 2.0
                else:
                    out[key] = p3d
    return out


def predict_mgfm(rows, model, mano, mesh_sampler, batch_size=32, tta=False):
    out = {}
    for tta_flip in ([False, True] if tta else [False]):
        patches, meta = [], []
        for r in rows:
        ...
        bs = batch_size
        for i in tqdm(range(0, len(patches), bs), desc=f"MGFM{'-TTA' if tta_flip else ''}"):
...
def main():
    print("[1/3] Loading Aria val + models…")
    ...
    print("[2/3] Predicting with TTA (orig + flip)…")
    mgfm_preds = predict_mgfm(aria_eval, mgfm_model, mgfm_mano, mgfm_mesh_sampler, tta=True)
    honet_preds = predict_honet(aria_eval, honet_model, tta=True)

    print("[3/3] Sweep ensemble weights with TTA…")
    from eval_runner import compute_metrics, run_metrics_aggregation
    for wmgfm in [0.7, 0.75, 0.8]:
        whonet = 1.0 - wmgfm
        samples = []
        for r in aria_eval:
            key = (r["sequence_name"], r["frame_id"], r["hand_side"])
        ...
        agg, _ = run_metrics_aggregation(samples, "aria_val", f"TTA-Ensemble {wmgfm}M+{whonet}H", True)
