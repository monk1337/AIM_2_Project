"""WiLoR FT with multi-teacher distillation + mesh-shape preservation.

Three supervision streams:
  ...
  - Both: vertex-L1 vs frozen off-shelf WiLoR reference (lambda_mesh)
          → prevents MANO pose from drifting to unnatural mesh-tangled
          configurations that score well on PA-MPJPE but render badly.
...
class AriaDistillDataset(Dataset):
    ...
    def __init__(self, image_size=256, augment=True, apply_reject_filter=True,
        ...
        if apply_reject_filter:
            ...
            R = json.load(open(_os.path.join(_sd, "reject_keys_all_20260419.json")))
...
def main():
    ...
    p.add_argument("--out_dir", default="/workspace/checkpoints/wilor_ft_distill_v2")
    ...
    p.add_argument("--lambda_mesh", type=float, default=0.5,
                   help="L1 weight on (pred_verts - off-shelf_verts), preserves MANO mesh shape")
    ...
    print("[1/5] Loading WiLoR pretrained (student + frozen ref)...")
    ...
    # Frozen off-shelf reference for mesh preservation
    pipe_ref = WiLorHandPose3dEstimationPipeline(device="cuda", dtype=torch.float32, verbose=False)
    model_ref = pipe_ref.wilor_model.eval()
    for p_ in model_ref.parameters():
        p_.requires_grad = False
    print(f"  ref model frozen, lambda_mesh={args.lambda_mesh}")
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            with autocast(dtype=torch.bfloat16):
                ...
                kp_loss, pov_l, aria_l = distill_loss(pred_3d, kp3d, kp3d_op, conf, src)
                with torch.no_grad():
                    ref_verts = model_ref(imgs)["pred_vertices"]
                # Centroid-anchored vertex L1: compares MESH SHAPE only (not
                # absolute camera-frame position), which is what distillation
                # is allowed to change. Penalizes pose drift away from off-shelf.
                pred_centered = pred_verts - pred_verts.mean(dim=1, keepdim=True)
                ref_centered = ref_verts - ref_verts.mean(dim=1, keepdim=True)
                mesh_l = (pred_centered - ref_centered).abs().mean()
                loss = kp_loss + args.lambda_mesh * mesh_l
            ...
            scaler.step(optim)
            scaler.update()
            pbar.set_postfix(loss=f"{float(loss):.4f}", pov=f"{float(pov_l):.3f}",
                             aria=f"{float(aria_l):.3f}", mesh=f"{float(mesh_l):.4f}")
            step += 1
