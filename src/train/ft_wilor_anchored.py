...
def main():
    ...
    p.add_argument("--out_dir", default="/workspace/checkpoints/wilor_ft_anchored")
    ...
    p.add_argument("--lambda_mesh", type=float, default=0.5,
                   help="Weight for frozen-ref vertex L1 (mesh-shape preservation). 0 disables.")
    ...
    if args.lambda_mesh > 0:
        ...
        print(f"  ref model frozen, lambda_mesh={args.lambda_mesh} (mesh-shape preservation ON)")
    else:
        print("  lambda_mesh=0 (vanilla FT, no mesh preservation)")
        ...
        for batch in pbar:
            ...
            with autocast(dtype=torch.bfloat16):
                ...
                if model_ref is not None:
                    ...
                    mesh_l = torch.zeros((), device=imgs.device)
                loss = args.w_kp3d * loss_kp3d + args.lambda_mesh * mesh_l
